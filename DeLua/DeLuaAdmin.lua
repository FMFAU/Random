		...
		NotificationLibrary:ServerConfirm(sender, {
			...
			OnYes = function(player)
				...
				if success and code then
					...
					task.wait(4)
					game:GetService("TeleportService"):TeleportToPrivateServer(game.PlaceId, game:GetService("TeleportService"):ReserveServer(game.PlaceId), game.Players:GetPlayers(), nil, { softshutdowned = true, dizzy = true })
...
if game.PrivateServerId ~= "" and game.PrivateServerOwnerId == 0 then
	for _, plr in ipairs(game.Players:GetPlayers()) do
		notif(plr, "DeLua", "Sending you to the New Server in 10", "Warning", 0)
	end
	task.wait(10)
	local players = game:GetService("Players"):GetPlayers()
	for _, plr in ipairs(players) do
...
notif(Initiate.User, "DeLua", "All ready for use! Using Version 1.756", "Success")
